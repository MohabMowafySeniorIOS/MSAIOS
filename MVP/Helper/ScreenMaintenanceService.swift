...
/// the Home screen and the dollar-prices screen are independent of each other
/// — and independent of `iosneedMaintain`, which still blocks the whole app
/// (see `HomeVC.getAppVersion()`).
///
/// The keys are iOS-specific on purpose: Android reads `homeMaintain` /
/// `dollarMaintain` from the same document, so a screen can be switched off on
