...
/// مكان عرض البانر — نفس القيم اللي السيرفر بيفهمها في `?placement=`.
///
/// السيرفر افتراضيه `popup` كمان، فالنسخ القديمة اللي مش بتبعت
/// البارامتر ده بتاخد نفس اللي كانت بتاخده بالظبط.
enum BannerPlacement: String {

    /// الإعلان اللي بيطلع في نص الرئيسية أول ما التطبيق يفتح.
    case popup

    /// الشريط الثابت في نهاية محتوى الرئيسية.
    case homeInline = "home_inline"
}
...
final class BannerService {
    ...
    static func fetchBanners(
        placement: BannerPlacement = .popup,
        completion: @escaping ([Banner]) -> Void
    ) {
        ...
        let query = "per_page=50&placement=\(placement.rawValue)"
        let url = base.contains("?") ? "\(base)&\(query)" : "\(base)?\(query)"
