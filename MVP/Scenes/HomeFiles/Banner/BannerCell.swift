...
final class BannerCell: UICollectionViewCell {
    ...
    private func configureAsImage(_ item: Banner) {
        ...
        imageView.kf.setImage(
            // نفس أندرويد: استخدم الغلاف، ولو مش موجود استخدم الميديا الأصلية.
            with: URL(string: item.thumbURL ?? item.mediaURL),
    ...
    private func configureAsVideo(_ item: Banner) {
        ...
        imageView.isHidden = false
        imageView.kf.setImage(
            // الغلاف اختياري؛ الميديا الأصلية هي fallback زي أندرويد.
            with: URL(string: item.thumbURL ?? item.mediaURL),
            placeholder: UIImage(named: "sliderPlaceHolder")
        )
