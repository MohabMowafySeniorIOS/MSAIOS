...
class OunceDollarCell: UITableViewCell {
    ...
    ///
    /// بتتحدّث في `configrationCell` (الدهب) **و**`configrationSilverCell`
    /// (الفضة)، وكل واحدة بتحط `metal` بتاعها — فالشاشة بتفتح على نفس
    /// المعدن اللي المستخدم شايفه في الرئيسية.
...
//              }
       ...
       // "\(trimNumber(number: gapPrice))"
        ...
        gapData = PriceGapData(
            ...
            localHighEgp: saleGold24Price,
            dollarSagha: dollarSaghaValue,
            dollarBank: dollarBankValue,
            gaugeValue: gab,
            gapHighAbs: abs(gab),
            gapHighIsNegative: gab < 0,
            gapLowAbs: abs(gap21Signed),
            gapLowIsNegative: gap21Signed < 0
        )
...
//                  }
    ...
    DollarLabel.text =  trimNumber(number: Double(calculateGoldDollarRate(
      ...
      print(ounce,Karat999,(Double(Dollar) ?? 0.0),buySilverPrice999)
        ...
        // MARK: - Gap screen data (الفضة)
        //
        // الجزء ده كان **ناقص تماماً**: `gapData` كانت بتتملا في
        // `configrationCell` (الدهب) بس. النتيجة إن المستخدم في وضع
        // الفضة يدوس على الفجوة فيلاقي أرقام دهب قديمة — الخلية
        // بتتعاد استخدامها فبتفضل شايلة بيانات آخر صف دهب — أو أصفار
        // لو الشاشة فتحت على الفضة من الأول.
        //
        // نفس معادلات الدهب بالظبط، بس الأساس عيار ٩٩٩ مش ٢٤:
        //   gap999 = |silver999Local − (worldPerGramUsd × dollarBank)|
        //   gap925 = |silver925Local − (worldPerGramUsd × 925/999 × dollarBank)|
        let silverDollarBank = Double(Dollar) ?? 0.0
        let silverWorldPerGramUsd = ounce > 0 ? ounce / 31.1035 : 0.0

        // «دولار الصاغة» في الفضة بيتحسب من سعر **الشراء** مش البيع —
        // زي ما `DollarLabel` فوق بيعمل بالظبط. لو استخدمنا سعر البيع
        // كان الرقم هيخالف اللي ظاهر في نفس الشاشة.
        let silverSagha = Double(calculateGoldDollarRate(
            ouncePriceUSD: ounce,
            localGramPrice24: Double(buySilverPrice) ?? 0.0
        )) ?? 0.0

        let silver925Local = buySilverPrice999 * (925.0 / 999.0)
        let world925InEgp = silverWorldPerGramUsd * (925.0 / 999.0) * silverDollarBank
        let gap925Signed = world925InEgp - silver925Local

        gapData = PriceGapData(
            metal: .silver,
            worldPerGramUsd: silverWorldPerGramUsd,
            localHighEgp: buySilverPrice999,
            dollarSagha: silverSagha,
            dollarBank: silverDollarBank,
            gaugeValue: gab,
            gapHighAbs: abs(gab),
            gapHighIsNegative: gab < 0,
            gapLowAbs: abs(gap925Signed),
            gapLowIsNegative: gap925Signed < 0
        )
